...
/*
 ・ブラウザで動かす場合は、index.htmlをブラウザで開いてください。
 ・Node.jsで実行する場合は npm で readline-sync をインストールしてから実行してください
    $ cd answer/prob2202
...
function readUserAnswer(count, message) {
  ...
  if (s === null || s === "-1" || s === "−１" || s === "ー１") {
     // キャンセルボタンの選択、あるいは-1の入力。全角にも対応
    return -1; // 終了する
  }
  s = zenkakuToHankaku(s); // 全角 -> 半角（数字）
...
// zenkakuToHankaku   ChatGPT（GPT-4）が作成してくれたバージョン
function zenkakuToHankaku(str) {
  const zenkakuNumbers = '０１２３４５６７８９';
  const hankakuNumbers = '0123456789';

  let result = ''; // 戻す文字列。後ろに順番に追加していく
  for (let char of str) {  // strの文字を1文字ずつチェック
    const index = zenkakuNumbers.indexOf(char); //zenkakuNumbersの中を検索
    if (index !== -1) { // あった
      result += hankakuNumbers[index]; // 対応するものに交換してresultの後ろに追加
    } else { // 見つからなかった
      result += char;
    }
  }
